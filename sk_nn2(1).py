...
from tensorflow.contrib.eager.python.metrics_impl import Accuracy
...
l1 = add_layer(xs,20, 64, activation_function=tf.nn.relu)
l2 = add_layer(l1, 64, 32, activation_function=tf.nn.relu)
#l3 = add_layer(l2, 512, 256, activation_function=tf.sigmoid)
#l4 = add_layer(l3, 256, 128, activation_function=tf.sigmoid)
prediction = add_layer(l2, 32, 10, activation_function = None)
...
with tf.name_scope('Accuracy'):
    ...
    tf.summary.scalar('Accuray',tensor=accuracy)
...
tmp_d = RNN_Trained.getDecadesProbFromRNN(np.arange(0, np.size(train_year)),min_length,train_timbres, train_pitches,train_year)
global_decades = tmp_d#songProcess.transfer_year_to_10d(tmp_d) 
tmp_d2 = RNN_Trained.getDecadesProbFromRNN(np.arange(0, np.size(cv_year)),min_length,cv_timbres, cv_pitches,cv_year) 
global_cv_decades =tmp_d2 #songProcess.transfer_year_to_10d(tmp_d2)
tmp_d3 = RNN_Trained.getDecadesProbFromRNN(np.arange(0, np.size(test_year)),min_length,test_timbres, test_pitches,test_year) 
global_test_decades =tmp_d3# songProcess.transfer_year_to_10d(tmp_d3)
...
for i in range(iteration):
    ...
    if i%50==0:
        saver.save(sess=sess, save_path="nn_music_model/nn_music.model", global_step=i) # 保存模型
        #y_pred = sess.run(pred, feed_dict=feed_dict_train)
        rs = sess.run(merged, {xs: batch_X, ys: songProcess.transfer_year_to_10d(batch_y)})
